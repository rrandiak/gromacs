...
void DumpBuilderPull::pr_pull_coord(DumpStrategy* strategy, int c, const t_pull_coord* pcrd)
{
    ...
    strategy->pr_named_value("start", EBOOL(pcrd->bStart));
...
void DumpBuilderPull::build(DumpStrategy* strategy)
{
    ...
    strategy->pr_named_value("pull-print-COM", EBOOL(pull->bPrintCOM));
    strategy->pr_named_value("pull-print-ref-value", EBOOL(pull->bPrintRefValue));
    strategy->pr_named_value("pull-print-components", EBOOL(pull->bPrintComp));
    strategy->pr_named_value("pull-nstxout", pull->nstxout);
    strategy->pr_named_value("pull-nstfout", pull->nstfout);
    strategy->pr_named_value("pull-pbc-ref-prev-step-com", EBOOL(pull->bSetPbcRefToPrevStepCOM));
    strategy->pr_named_value("pull-xout-average", EBOOL(pull->bXOutAverage));
    strategy->pr_named_value("pull-fout-average", EBOOL(pull->bFOutAverage));
...
void DumpBuilderAwh::pr_awh_bias(DumpStrategy* strategy, const gmx::AwhBiasParams& awhBiasParams, const char* prefix)
{
    ...
    sprintf(opt, "%s-target-metric-scaling", prefix);
    strategy->pr_named_value(opt, EBOOL(awhBiasParams.scaleTargetByMetric()));
    sprintf(opt, "%s-target-metric-scaling-limit", prefix);
    strategy->pr_named_value(opt, awhBiasParams.targetMetricScalingLimit());
    sprintf(opt, "%s-user-data", prefix);
    strategy->pr_named_value(opt, EBOOL(awhBiasParams.userPMFEstimate()));
    sprintf(opt, "%s-share-group", prefix);
    strategy->pr_named_value(opt, awhBiasParams.shareGroup());
    sprintf(opt, "%s-equilibrate-histogram", prefix);
    strategy->pr_named_value(opt, EBOOL(awhBiasParams.equilibrateHistogram()));
    sprintf(opt, "%s-ndim", prefix);
...
void DumpBuilderAwh::build(DumpStrategy* strategy)
{
    ...
    strategy->pr_named_value("awh-share-bias-multisim", EBOOL(awhParams->shareBiasMultisim()));
...
void DumpBuilderRot::pr_rotgrp(DumpStrategy* strategy, int g, const t_rotgrp* rotg)
{
    ...
    strategy->pr_named_value("rot-massw", EBOOL(rotg->bMassW));
...
void DumpBuilderFep::build(DumpStrategy* strategy)
{
    ...
    strategy->pr_named_value("sc-coul", EBOOL(fep->bScCoul));
...
void DumpBuilderExpanded::build(DumpStrategy* strategy)
{
    ...
    strategy->pr_named_value("symmetrized-transition-matrix", EBOOL(expanded->bSymmetrizedTMatrix));
    ...
    strategy->pr_named_value("wl-oneovert", EBOOL(expanded->bWLoneovert));

    strategy->pr_rvec("init-lambda-weights", expanded->init_lambda_weights.data(), n_lambda, TRUE);
    strategy->pr_named_value("init-weights", EBOOL(expanded->bInit_weights));
}
...
void DumpBuilderSwap::build(DumpStrategy* strategy)
{
    ...
    {
        snprintf(str, STRLEN, "massw_split%d", j);
        strategy->pr_named_value(str, EBOOL(swap->massw_split[j]));
        snprintf(str, STRLEN, "split atoms group %d", j);
...
void DumpBuilderQmOpts::build(DumpStrategy* strategy) {
    ...
    strategy->pr_named_value("continuation", EBOOL(ir->bContinuation));

    strategy->pr_named_value("Shake-SOR", EBOOL(ir->bShakeSOR));
    ...
    /* COM PULLING */
    strategy->pr_named_value("pull", EBOOL(ir->bPull));
    if (ir->bPull) {
        DumpBuilderPull(ir->pull.get()).build(strategy);
    }

    /* AWH BIASING */
    strategy->pr_named_value("awh", EBOOL(ir->bDoAwh));
    if (ir->bDoAwh) {
        DumpBuilderAwh(ir->awhParams.get()).build(strategy);
    }

    /* ENFORCED ROTATION */
    strategy->pr_named_value("rotation", EBOOL(ir->bRot));
    if (ir->bRot) {
        DumpBuilderRot(ir->rot.get()).build(strategy);
    }

    /* INTERACTIVE MD */
    strategy->pr_named_value("interactiveMD", EBOOL(ir->bIMD));
    if (ir->bIMD) {
    ...
    strategy->pr_named_value("disre-mixed", EBOOL(ir->bDisreMixed));
    ...
    /* SIMULATED TEMPERING */
    strategy->pr_named_value("simulated-tempering", EBOOL(ir->bSimTemp));
    if (ir->bSimTemp)
    {
...
void DumpBuilderGrpOpts::build(DumpStrategy* strategy) {
    ...
    // }
    // fprintf(out, "\n");
    strategy->pr_rvecs("acc", opts->acceleration, opts->ngacc);
    ...
    // }
    // fprintf(out, "\n");
    strategy->pr_ivecs("nfreeze", opts->nFreeze, opts->ngfrz);
...
void DumpBuilderInputRec::build(DumpStrategy* strategy) {
    ...
    if (strategy->available(ir, title)) {
        ...
        strategy->pr_named_value("periodic-molecules", EBOOL(ir->bPeriodicMols));
        ...
        strategy->pr_named_value("print-nose-hoover-chain-variables", EBOOL(ir->bPrintNHChains));
