...
public:
    YamlStrategy(FILE* file_pointer);
    ~YamlStrategy();
    ...
    void pr_ap_vec_row(std::string title, const float vec[], int n, int indexndex) override;
    ...
    void pr_list_i(std::string title, int indexndex, gmx::ArrayRef<const int> list) override;
    ...
    void pr_interaction_list(std::string title, const t_functype* functypes, const InteractionList& ilist, const t_iparams* iparams) override;
