...
public:
    ...
    FormatterComponent(FILE* fp, int indent) : fp(fp), indent(indent) {}
    ~FormatterComponent() { fflush(fp); }
...
public:
    ...
    virtual void pr_grp_opt_agg(const rvec acceleration[],
                                int        ngacc,
                                const ivec nFreeze[],
                                int        ngfrz,
                                const int  egp_flags[],
    ...
    virtual void pr_interaction_list(std::string            title,
                                     const t_functype*      functypes,
                                     const InteractionList& ilist,
                                     const t_iparams*       iparams) = 0;
