...
void TopologyRepr::build(ReprFormatter* strategy)
{
    if (!strategy->bSysTop)
    {
        MTopRepr(mtop).build(strategy);
    }
    else
    {
        t_topology top = gmx_mtop_t_to_t_topology(mtop, false);
        TopRepr(&top).build(strategy);
    }
}

void MTopRepr::build(ReprFormatter* strategy)
{
    ...
    if (!(mtop->moltype.empty()))
    {
        ...
        {
            MoltypeRepr(&(mtop->moltype[mt]), mt, mtop->ffparams).build(strategy);
        }
        strategy->close_list();
    }

    SimulationGroupsRepr(mtop->groups).build(strategy);
    strategy->close_section();
}

void TopRepr::build(ReprFormatter* strategy)
{
    ...
    if (strategy->available(&top->idef, "idef"))
    {
        ...
        for (int j = 0; (j < F_NRE); j++)
        {
            ...
            strategy->pr_interaction_list(
                    interaction_function[j].longname, top->idef.functype, iList, top->idef.iparams);
        }
    }
}

void AtomsRepr::build(ReprFormatter* strategy)
{
...
void FFParamsRepr::build(ReprFormatter* strategy)
{
...
void ListOfListsRepr::build(ReprFormatter* strategy)
{
...
void MolblockRepr::build(ReprFormatter* strategy)
{
    ...
    for (size_t mb = 0; mb < molblock.size(); mb++)
    {
        ...
        {
            strategy->pr_rvecs("posres_xA",
                               as_rvec_array(molblock[mb].posres_xA.data()),
                               molblock[mb].posres_xA.size());
        }
        strategy->pr_count("posres_xB", molblock[mb].posres_xB.size());
        if (!molblock[mb].posres_xB.empty())
        {
            strategy->pr_rvecs("posres_xB",
                               as_rvec_array(molblock[mb].posres_xB.data()),
                               molblock[mb].posres_xB.size());
        }
        strategy->close_section();
    }
    strategy->close_list();
...
void MoltypeRepr::build(ReprFormatter* strategy)
{
    strategy->pr_title_i("moltype", index);
    strategy->pr_attribute_quoted("name", *(moltype->name));
    AtomsRepr(&(moltype->atoms)).build(strategy);
    ListOfListsRepr("excls", moltype->excls).build(strategy);
    for (int i = 0; (i < F_NRE); i++)
    {
        strategy->pr_interaction_list(interaction_function[i].longname,
...
void SimulationGroupsRepr::build(ReprFormatter* strategy)
{
