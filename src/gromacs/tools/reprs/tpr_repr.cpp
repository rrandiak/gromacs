...
#include "gromacs/tools/reprs/reprs/grp_opts.h"
#include "gromacs/tools/reprs/reprs/grp_stats.h"
#include "gromacs/tools/reprs/reprs/inputrec.h"
#include "gromacs/tools/reprs/reprs/qm_opts.h"
#include "gromacs/tools/reprs/reprs/topology.h"
#include "gromacs/tools/reprs/reprs/tpx_header.h"

TprRepr::TprRepr(const char*             filename,
                 const char*             mdpFilename,
                 gmx_bool                bOriginalInputrec,
                 std::vector<TprSection> sections) :
    filename(filename),
    mdpFilename(mdpFilename),
...
void TprRepr::build(ReprFormatter* strategy)
{
...
void TprRepr::buildSection(TprSection section, ReprFormatter* strategy)
{
    if (!sections.empty())
    {
    ...
    switch (section)
    {
        case TprSection::InputRec: InputRecRepr(tpx.bIr ? &ir : nullptr).build(strategy); break;
        case TprSection::QmOptions: QmOptionsRepr(&ir).build(strategy); break;
        case TprSection::GroupOptions: GroupOptionsRepr(&(ir.opts)).build(strategy); break;
        case TprSection::TpxHeader: TpxHeaderRepr(&tpx).build(strategy); break;
        case TprSection::Topology: TopologyRepr(&mtop).build(strategy); break;
        case TprSection::Box: strategy->pr_rvecs("box", tpx.bBox ? state.box : nullptr, DIM); break;
        ...
        case TprSection::GroupStats:
            GroupStatsRepr(&mtop.groups, mtop.natoms).build(strategy);
            break;
        default: break;
    }
}
